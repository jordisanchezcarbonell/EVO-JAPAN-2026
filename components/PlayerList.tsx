...
export function PlayerList({ players }: { players: PlayerWithResults[] }) {
  ...
  const filtered = useMemo(() => {
    return players.filter((p) => {
      if (game !== "ALL" && !p.events.some((e) => e.eventName === game)) return false;
      if (query) {
        const q = query.toLowerCase();
        const hay = `${p.gamertag} ${p.realName ?? ""} ${p.city ?? ""}`.toLowerCase();
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex flex-wrap gap-2">
          ...
          {games.map((g) => {
            const count = players.filter((p) => p.events.some((e) => e.eventName === g)).length;
            return (
        ...
        <input
          ...
          className="w-full rounded-none border border-[var(--color-line)] bg-[var(--color-surface)] px-3 py-2 font-mono text-sm text-[var(--color-fg)] placeholder-[var(--color-muted)] focus:border-[var(--color-amarillo)] focus:outline-none sm:w-72"
        />
      </div>
      ...
      <div className="overflow-x-auto border border-[var(--color-line)]">
        <table className="w-full border-collapse text-sm">
          ...
          <tbody>
                ...
                <Fragment key={p.id}>
                  <tr
                    ...
                    className={`border-b border-[var(--color-line)] transition-colors hover:bg-[var(--color-surface-2)] cursor-pointer ${
                      i % 2 === 0 ? "bg-transparent" : "bg-[var(--color-surface)]/40"
                  ...
                  >
                    ...
                    <td className="px-4 py-3 font-mono text-xs text-[var(--color-muted)]">
                      {p.city ? `${p.city}${p.state ? `, ${p.state}` : ""}` : <Dim />}
                    </td>
                    <td className="px-4 py-3">
                    ...
                    <td className="px-4 py-3">
                      <div className="flex flex-wrap gap-1">
                        {p.standings.length > 0
                          ...
                          : <span className="font-mono text-[10px] text-[var(--color-line)]">—</span>
                        }
                      </div>
                    </td>
                  ...
                  {isExpanded && (
                    <tr key={`${p.id}-detail`} className="border-b border-[var(--color-line)]">
            ...
            {filtered.length === 0 && (
              <tr>
                <td colSpan={6} className="px-4 py-8 text-center font-mono text-sm text-[var(--color-muted)]">
                  sin resultados
                </td>
              </tr>
            )}
